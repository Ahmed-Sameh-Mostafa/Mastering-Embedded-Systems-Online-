 ...
 * @brief			- It is used when i want to change status of certain group of bins using a special mask
 * @param [in] 		- GPIOx: where x can be (A..D depending on device used) to select GPIO peripheral
 * @param [in] 		- Value: New Port Value
 * @param [in] 		- Mask: it is mask used to define which bits in the port will be changed and the rest will remain unchanged
 * @retval 			- OK if pin configure is locked Or ERROR if pin is not locked (OK & ERROR are defined @ref GPIO_RETURN_LOCK)
 * Note				- None
 */
void MCAL_GPIO_WriteGroup(GPIO_TypeDef * GPIOx,uint16_t Value,uint16_t mask)
{
	GPIOx->ODR &= ~mask; // reset the values of the group as new Value of a bin of the group can be 1 or 0
	GPIOx->ODR |= (Value & mask); // we make sure the new Value will not change any pins outside the group before assigning the new value to the port
}

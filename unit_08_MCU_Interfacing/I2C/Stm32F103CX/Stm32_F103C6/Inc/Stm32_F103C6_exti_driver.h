...
typedef struct
{
	EXTI_GPIO_Mapping_t 	EXTI_PIN; // Specifies the external interrupt GPIO mapping.
									  // takes one of the values from @ref EXTI_define

	uint8_t 				Trigger_Case; // Specifies if the trigger is falling edge, rising edge or both
									  // takes one of the values from @ref EXTI_Trigger_define

	uint8_t 				IRQ_EN;		// Enable or Disable the EXTI IRQ
										/// takes one of the values from @ref EXTI_IRQ_define
